    ...
    <aside class="w-56 bg-stone-900 flex flex-col flex-shrink-0 h-screen sticky top-0 overflow-y-auto">
        ...
        <nav class="flex-1 px-3 py-4 space-y-1">
            ...
            @if(!auth()->user()->isAdmin())
                ...
                <a href="{{ route('cashier.pos') }}" @class([
                    'flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors',
                    'bg-amber-500/20 text-amber-400' => request()->routeIs('cashier.pos'),
                    'text-white/60 hover:text-white hover:bg-white/8' => !request()->routeIs('cashier.pos'),
                ])>
